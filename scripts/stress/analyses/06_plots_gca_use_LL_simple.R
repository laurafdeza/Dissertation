...
# Load models 
load(paste0(gca_mods_path, "/gca_mon_mods_simple.Rdata"))
...
use_plot <- model_preds_simple$fits_all_use %>%
...
ggsave(paste0(figs_path, "/use_plot_simple.png"), use_plot, width = 180,
       height = 120, units = "mm", dpi = 600)


# model_preds_simple$fits_all_use %>%
#   mutate(`Spanish use` = as.factor(use_z),
#          Stress = if_else(condition_sum == 1, "Oxytone", "Paroxytone"),
#          Stress = fct_relevel(Stress, 'Paroxytone'),
#          L1 = if_else(l1_sum == 1, "Mandarin Chinese", "English"),
...
stress_mon <- model_preds_simple$fits_all_mon %>%
...
ggsave(paste0(figs_path, "/mon_plot_simple.png"), stress_mon, width = 180,
       height = 120, units = "mm", dpi = 600)
