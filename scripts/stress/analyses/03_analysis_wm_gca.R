...
verbalwm <- read_csv("./data/clean/ospan_set_z_scores.csv")

stress50 <- left_join(stress50, verbalwm, by = "participant")
...
gca_full_mod_ospan_0 <- update(gca_full_mod_stress_2,   . ~ . + ospan) 
gca_full_mod_ospan_1 <- update(gca_full_mod_ospan_0, . ~ . + ot1:ospan) 
gca_full_mod_ospan_2 <- update(gca_full_mod_ospan_1, . ~ . + ot2:ospan) 
gca_full_mod_ospan_3 <- update(gca_full_mod_ospan_2, . ~ . + ot3:ospan) 

full_ospan_anova <-
  anova(gca_full_mod_stress_2, gca_full_mod_ospan_0, gca_full_mod_ospan_1,
        gca_full_mod_ospan_2, gca_full_mod_ospan_3)
#                       npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# gca_full_mod_stress_2   36 239087 239399 -119508   239015                     
# gca_full_mod_ospan_0    37 239089 239410 -119508   239015 0.0000  1     0.9959
# gca_full_mod_ospan_1    38 239091 239420 -119508   239015 0.0175  1     0.8949
# gca_full_mod_ospan_2    39 239092 239430 -119507   239014 1.4003  1     0.2367
# gca_full_mod_ospan_3    40 239093 239440 -119507   239013 0.1567  1     0.6922


# add 2-way int to intercept, linear slope, quadratic, and cubic time terms
gca_full_mod_int_0 <- update(gca_full_mod_stress_2, . ~ . + group:condition_sum:ospan) 
gca_full_mod_int_1 <- update(gca_full_mod_int_0, . ~ . + ot1:group:condition_sum:ospan) 
gca_full_mod_int_2 <- update(gca_full_mod_int_1, . ~ . + ot2:group:condition_sum:ospan) 
gca_full_mod_int_3 <- update(gca_full_mod_int_2, . ~ . + ot3:group:condition_sum:ospan) 

full_int_anova <- anova(gca_full_mod_stress_2, gca_full_mod_int_0, gca_full_mod_int_1, 
                         gca_full_mod_int_2, gca_full_mod_int_3)
#                       npar    AIC    BIC  logLik deviance   Chisq Df Pr(>Chisq)
# gca_full_mod_stress_2   36 239087 239399 -119508   239015                      
# gca_full_mod_int_0      41 239092 239448 -119505   239010  4.7635  5   0.445419
# gca_full_mod_int_1      46 239046 239445 -119477   238954 56.4799  5  6.472e-11
# gca_full_mod_int_2      51 239039 239482 -119469   238937 16.4643  5   0.005636
# gca_full_mod_int_3      56 239034 239519 -119461   238922 15.7370  5   0.007637
...
gca_full_mods_wm <- mget(c(paste0(mod_type, mod_spec)))    # int_2 final model
  
save(gca_full_mods_wm,
     file = here("mods", "stress", "gca",
                   "gca_full_mods_wm.Rdata"))
...
nested_model_comparisons_wm <-
  mget(c("full_ospan_anova", "full_int_anova"
         ))
