...
load(paste0(gca_mods_path, "/relevel_final_mods.Rdata"))
...
# movement anticipation through vision and verbal and visuospatial WM
...
model_preds$target_offset_preds %>% 
  mutate(cond = if_else(stress == 1, "paroxytone", "oxytone")) %>% 
...
pretty_fixed_effects <- group_mods$gca_full_mod_group_2 %>% 
  tidy_lme4() %>% 
...
ranef_table <- group_mods$gca_full_mod_group_2 %>% 
  tidy_ranef_summary %>% 
  # Format the numbers
...
pairwise_comp <- relevel_final_mods$gca_full_mod_aes_relevel %>% 
  tidy_lme4 %>% 
  rename(B = Estimate) %>% 
...
pretty_pairwise_table <- relevel_final_mods$gca_full_mod_aes_relevel %>% 
  tidy_lme4 %>% 
  mutate(p = format_pval(p), 
         Parameter = fix_param_names(Parameter)) %>% 
  mutate_each(funs(format_fixef_num), Estimate:t) %>% 
...
pw_subs <- c(paste0(0:0, 8), paste0(0:1, 9))
...
# make table for printing
pairwise_comp <- relevel_final_mods$gca_full_mod_ams_relevel %>% 
  tidy_lme4 %>% 
  rename(B = Estimate) %>% 
...
# Make table for appendix
pretty_pairwise_table <- relevel_final_mods$gca_full_mod_ams_relevel %>% 
  tidy_lme4 %>% 
  mutate(p = format_pval(p), 
         Parameter = fix_param_names(Parameter)) %>% 
  mutate_each(funs(format_fixef_num), Estimate:t) %>% 
...
# make table for printing
pairwise_comp <- relevel_final_mods$gca_full_mod_ies_relevel %>% 
  tidy_lme4 %>% 
  rename(B = Estimate) %>% 
  filter(str_detect(Parameter, "ims") == TRUE) 
...
```{r, pairwise-comp-table, results='asis', eval=T}
# Make table for appendix
pretty_pairwise_table <- relevel_final_mods$gca_full_mod_ies_relevel %>% 
  tidy_lme4 %>% 
  mutate(p = format_pval(p), 
         Parameter = fix_param_names(Parameter)) %>% 
  mutate_each(funs(format_fixef_num), Estimate:t) %>% 
...
params <- relevel_final_mods$gca_full_mod_ams_relevel %>% 
  tidy_lme4() %>% 
  rename(B = Estimate)
