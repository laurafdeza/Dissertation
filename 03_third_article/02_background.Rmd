...
Hence, if in languages like Spanish a listener hears "los," they will expect the following noun to be also masculine and plural, such as "niños" (the children~masc~).
These associations facilitate language processing, because the hearer anticipates outcomes based on available information [e.g., @federmeier2007thinking; @wicha2003potato], and therefore does not need to pay close attention to all information coming in and can make up for the information that is lost due to external reasons, such as noise.
...
These two types of models make opposite predictions as to how associations in learning are created and how abilities in other cognitive domains influence acquisition of skills in learning.

The goal of this paper is to study transfer of anticipatory skills from the visual domain to speech, to acquire and process associations of lexical stress-verb tense suffix.
This mapping is difficult to acquire, and even proficient L2 speakers have issues in using it for easier processing [@sagarra2018suprasegmental].
...
While monolingual speakers generate predictions constantly, it requires time to acquire an L2 and develop the ability to generate predictions in that L2 [@lew2010real; @sagarra2018suprasegmental].
...
In learning settings, transfer that occurs between two closely related domains is called near-transfer; when transfer occurs between distant domains, it is far-transfer [@mestre2006transfer].
...
In addition, the more developed a skill is, the more domain-specific the features will be, reducing the likelihood of transfer [@ericsson1994expert; @gobet2015understanding].
...
A large body of research has also focused on the enhancement of these domain-general mechanisms through training of domain-specific skills (e.g., chess, music, for reviews, see @sala2017far; @simons2016brain; @strobach2016cognitive).
...
Thus, languages can talk about space in egocentric or in geocentric terms [@levinson1997language], affecting our spatial reasoning [@levinson2002returning].
...
Over time, individuals with dyslexia tend to perform similarly to typical individuals in many visuospatial tasks [@karolyi2004dyslexia].
Research on blind individuals has also shown that traditionally visual brain regions are recruited during verbal tasks such as Braille reading (e.g., @kupers2007rtms; @uhl1991functionality), verb generation in response to nouns (e.g., @amedi2003early; @burton2002adaptive) and sentence comprehension (e.g., @bedny2011language; @roder2000event), although only when the individual was already blind as a child (@bedny2012sensitive).
...
In the operation span task, participants need to remember words while being distracted by mathematical additions and substractions.
In the Corsi test, participants need to remember sequences of flashing squares.

Previous studies revealed that L1 Spanish speakers, Mandarin and English advanced learners of Spanish and intermediate English speakers can generate predictions for tense using lexical stress in CVC syllables, especially when immersed in the L2 [@PRIMER EXP], we therefore hypothesized that these groups would have no issues in this study either.  
In the L1 Spanish speakers, no relationship was expected between visuospatial anticipation and speech anticipation. 
Our reasoning is that the two modalities, auditory (speech) and visual, are two different for there to be any transfer, and thus the "potentially transferable" skills would be two specific to each domain [@ericsson1994expert; @gobet2015understanding].
Verbal WM is not expected to be associated either [@sagarra2018suprasegmental; @PRIMER EXP], due to L1 speakers' practice in generating the predictions overriding WM differences.   
Visuospatial WM is, for the same reason, not predicted to have any influence.
...
Therefore, it would not matter what the L1 or the L2 is, or the proficiency in the L2: increased abilities to anticipate movements in space through vision will not be associated with speech in any way. 
...
These findings would thus support a domain-specific learning mechanism.
