...
The log odds of participants fixating on the target at 200 ms after the objective onset of the last syllable in the verb estimated by the processing speeds model is `r report_row(0)` (proportion = `r print(b0_prop)`).
...
This interaction further informs us that when participants had faster verbal processing speed but slower visuospatial one, they were more slowy in verbal prediction rate in paroxytones, but oxytones were negatively affected mostly when both processing speeds were slow (`r report_row(10)`) (see Figure 4).
