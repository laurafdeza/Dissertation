...
pretty_pairwise_table <- full_mods_refactor$gca_full_mod_int_2_ams %>% 
  tidy_lme4 %>% 
  mutate(p = format_pval(p), 
         Parameter = fix_param_names(Parameter)) %>% 
  mutate_each(funs(format_fixef_num), Estimate:t) %>% 
  rename(`_t_` = t, `_p_` = p) %>% 
  filter(str_detect(Parameter, "AE") == TRUE) 

pretty_pairwise_table$Parameter %<>% str_replace("Group AM", "AM - AE")
...
# make table for printing
pairwise_comp <- full_mods_refactor$gca_full_mod_int_2_aes %>% 
  tidy_lme4 %>% 
  rename(B = Estimate) %>% 
...
```{r, pairwise-comp-table, results='asis', eval=T}
# Make table for appendix
pretty_pairwise_table <- full_mods_refactor$gca_full_mod_int_2_aes %>% 
  tidy_lme4 %>% 
  mutate(p = format_pval(p), 
         Parameter = fix_param_names(Parameter)) %>% 
  mutate_each(funs(format_fixef_num), Estimate:t) %>% 
...
pretty_pairwise_table$Parameter %<>% str_replace("Group AE", "AE - IE")
...
# make table for printing
pairwise_comp <- full_mods_refactor$gca_full_mod_int_2_ies %>% 
  tidy_lme4 %>% 
  rename(B = Estimate) %>% 
  filter(str_detect(Parameter, "ims") == TRUE) 
...
```{r, pairwise-comp-table, results='asis', eval=T}
# Make table for appendix
pretty_pairwise_table <- full_mods_refactor$gca_full_mod_int_2_ies %>% 
  tidy_lme4 %>% 
  mutate(p = format_pval(p), 
         Parameter = fix_param_names(Parameter)) %>% 
  mutate_each(funs(format_fixef_num), Estimate:t) %>% 
