...
load(paste0(gca_en_mods_path, "/gca_en_mods_verb.Rdata"))
...
knitr::include_graphics(
  here("figs", "use_prof", "timecourse_all.png")
)
```
...
params <- gca_en_mods_verb$gca_en_mod_final %>%
  tidy_lme4() %>% 
  rename(B = Estimate)
...
# Get full mod and make it pretty for the table
pretty_fixed_effects <- gca_en_mods_verb$gca_en_mod_final %>%
  tidy_lme4() %>% 
  mutate(p = format_pval(p), 
         Parameter = fix_param_names(Parameter)) %>% 
  mutate_each(funs(format_fixef_num), Estimate:t) %>% 
...
probs_l2 <- model_preds_en_verb$target_offset_preds_en %>%
...
knitr::include_graphics(
  here("figs", "use_prof", "gca_verb", "en_gca_all_verb.png")
)
```
...
ranef_table <- gca_en_mods_verb$gca_en_mod_final %>% 
  tidy_ranef_summary %>% 
  # Format the numbers
  mutate_each(funs(format_fixef_num), vcov, sdcor) %>%
  mutate_each(funs(format_cor), -var1, -grp, -vcov, -sdcor) %>%
